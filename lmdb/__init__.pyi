...
@overload
def version(*, lib_version: int = ...) -> tuple[int, int, int]: ...
@overload
def version(
    *, subpatch: Literal[False], lib_version: int = ...
) -> tuple[int, int, int]: ...
@overload
def version(
    *, subpatch: Literal[True], lib_version: int = ...
...
# LMDB 1.0.x error codes; absent when built against a 0.9 system liblmdb.
...
class Environment:
    ...
    def close(self) -> None: ...
    def path(self) -> str: ...
    def lib_version(self) -> tuple[int, int, int]: ...
    def copy(
...
class Transaction(Generic[_VT_co]):
    ...
    def cursor(self, db: _Database | None = None) -> Cursor[_VT_co]: ...
